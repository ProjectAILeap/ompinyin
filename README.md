...
> 在 **Omarchy** 上**一键部署中文输入法**：**雾凇全拼 + 万象整句模型 + 顶栏输入法图标 + 候选框跟随主题配色**，双拼按需可选。

`ompinyin` 把「装好中文输入法」变成一条**可反复执行**的命令：声明目标终态（全拼 + 万象 + 顶栏图标 + 候选框跟随主题）→ 采集现状 → 只改不同部分。随时重跑、`--dry-run` 预览、`uninstall` 回滚。
