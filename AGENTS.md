...
- X11 HiDPI（`--x11-hidpi`）是**显式 opt-in 的兼容模式**，默认只诊断：`Xft.dpi` 是全局 XWayland 资源，可能二次放大其它 X11/Electron 应用；混合 DPI 多屏无单值精确解、不做焦点切换自动改写。撤销走 `--no-x11-hidpi`（移除受管块 + 两个 user 单元），不在 stop 窗口内做。
